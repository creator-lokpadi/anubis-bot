...
module.exports = {
    async before(m) {
        ...
        if (m.quoted.id == this.tebakkata[id][0].id) {
            ...
            if (m.text.toLowerCase() == json.anubis.jawaban.toLowerCase().trim()) {
                global.db.data.users[m.sender].exp += this.tebakkata[id][2]
                m.reply(`*Benar!*\n+${this.tebakkata[id][2]} XP`)
                clearTimeout(this.tebakkata[id][3])
                delete this.tebakkata[id]
            } else if (similarity(m.text.toLowerCase(), json.anubis.jawaban.toLowerCase().trim()) >= threshold) m.reply(`*Dikit Lagi!*`)
            else m.reply(`*Salah!*`)
        }
        return !0
    },

    ...
    try {
      ...
      try {
        ...
        if (chat) {
          ...
          if (!('bdwrd' in chat)) chat.bdwrd = false
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
            this.send2Button(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}buy*`, '© anubis-BOT', 'Buy', `${usedPrefix}buy`, 'Buy All', `${usedPrefix}buyall`, m)
...
*.enable delete*
`.trim(), '© anubis-BOT', 'Matikan', '.1 delete', m.message)
